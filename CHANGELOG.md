...
* `KeyProvider` interface with `EmbeddedKeyProvider` (XOR part-combining helper), `CallbackKeyProvider`, `FallbackKeyProvider`, and `RemoteKeyProvider` (HTTPS fetch with per-request headers, retries, single-flight, and a pluggable `KeyCache`; `InMemoryKeyCache` included)
