...
* CLI: `dart run litert_crypto init | keygen | encrypt` (direct arguments or a `litert_crypto.yaml` config). With `key_parts_out`, `encrypt` also generates the Dart source for `EmbeddedKeyProvider`, so the embedded key cannot drift from the key the models were encrypted with; it rewrites that file only when the key changed
